import { CLOSE_AUTH_FAILED, CLOSE_REVOKED, CLOSE_UNSUPPORTED_VERSION, HEARTBEAT_TIMEOUT_MS, MAX_BODY_CHUNK_BYTES, MAX_CONTROL_FRAME_BYTES, MAX_REQUEST_BODY_BYTES, MAX_WS_MESSAGE_BYTES, PROTOCOL_VERSION, RELAY_AGENT_PATH, agentChallenge, fingerprint, hashToken, isAllowedWebSocketPath, isSafeProxyPath, parseFrame, rewriteRequestHeaders, rewriteResponseHeaders, signMessage } from "./headers-BoJKLZo1.js";
import { defaultCredentialPath, dshHome, openCredentials, openStatus, resolvePluginConfig, stateDir, statusPath } from "./config-ulTq7JKy.js";
	...
	async handlePlainRequestForTest(rid, path) {
		...
		return await new Promise((resolve) => {
			...
			this.#testSink = (frame) => {
				...
				if (frame.type === "body.chunk" && frame.rid === rid) body += Buffer.from(frame.data, "base64").toString("utf8");
