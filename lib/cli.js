#!/usr/bin/env node
import { decodePairingCode, encodePairingCode, fingerprint, hashToken } from "./headers-BZAfeW6Z.js";
import { defaultCredentialPath, openCredentials, openStatus, stateDir, statusPath } from "./config-C9YP7cr1.js";
import { DEFAULT_RELAY_OPTIONS, RelayStore, startRelay } from "./server-Dr3wnIIu.js";
...
async function commandRelay(args) {
	...
	if (active.length === 0 && !args.booleans.has("no-auto-register")) {
		...
		const publicUrl = relayUrlFor(args) ?? `http${secure ? "s" : ""}://${host === "0.0.0.0" ? "YOUR-RELAY-HOST" : host}:${String(relay.port)}`;
		say();
