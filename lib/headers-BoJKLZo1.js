...
import { createHash, createPrivateKey, createPublicKey, generateKeyPairSync, randomBytes, randomUUID, sign, timingSafeEqual, verify } from "node:crypto";
	...
	*/
	...
	#read() {
		...
		try {
			...
			throw new Error(`${this.#path} is unreadable (${detail}). Fix or remove the file before continuing.`);
		}
	...
	*/
	#lock() {
		...
		for (;;) {
			...
			if (Date.now() > deadline) throw new Error(`timed out waiting for ${dir}. Another process is writing; remove that directory if none is.`);
...
//#endregion
export { AUTH_PROOF_SKEW_MS, CLOSE_AGENT_OFFLINE, CLOSE_AGENT_REPLACED, CLOSE_AUTH_FAILED, CLOSE_RATE_LIMITED, CLOSE_REVOKED, CLOSE_UNSUPPORTED_VERSION, HEARTBEAT_INTERVAL_MS, HEARTBEAT_TIMEOUT_MS, MAX_BODY_CHUNK_BYTES, MAX_CONTROL_FRAME_BYTES, MAX_REQUEST_BODY_BYTES, MAX_WS_MESSAGE_BYTES, OneTimeValues, PROTOCOL_VERSION, RELAY_AGENT_PATH, RateLimiter, WatchedFile, agentChallenge, decodePairingCode, encodePairingCode, fingerprint, generateAgentIdentity, generateEncryptionToken, generateToken, hashToken, isAllowedWebSocketPath, isSafeProxyPath, normalizeHeaders, parseFrame, rewriteRequestHeaders, rewriteResponseHeaders, safeEqual, signMessage, verifySignature };